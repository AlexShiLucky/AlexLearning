...
cpu_bool_t crond_autopowerup_calc(crond_t *pcrond)
{
    sys_time_t *nt;
    crontab_t *pct;
    struct _tm *alarmt = NULL;
    struct _tm now;
    ...
    if (nt->flags & (DEF_FLAGS_TIME_INIT|DEF_FLAGS_TIME_SYNC)) {
        ...
        if (alarmt) {
            LOG_Print(VTANSI_COLOR_LIGHT_GREEN"[POWERUP-Alarm!!!] Now:%u-%02u-%02u %s %02u:%02u:%02u"VTANSI_DISPLAY_RESET""CONFIG_LINE_ENDING, \
                    alarmt->year+BASE_YEAR, alarmt->mon+1, alarmt->mday+1,     \
                    &wdays_str[alarmt->wday*4], alarmt->hour, alarmt->min, alarmt->sec);
            DELIMITER_LINE();
        }
    }
...
cpu_bool_t crond_autoshutoff_calc(crond_t *pcrond)
{
            ...
            if (nsecs >= priv->shutoff_active_nsecs) {
                if (1) {     // 如果USB断开连接,则通知时序自动关机
                    if (sys_settime(nt, OSTimeGet()) == DEF_Success) {
                        sys_gettime(nt, &rtc);
                        LOG_Print(VTANSI_COLOR_LIGHT_RED"[SHUTOFF-Alarm!!!] Now:%u-%02u-%02u %s %02u:%02u:%02u"VTANSI_DISPLAY_RESET""CONFIG_LINE_ENDING, \
