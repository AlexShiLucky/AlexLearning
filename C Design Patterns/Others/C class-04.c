#include <stdio.h>
// 父类
typedef struct _shape_ops shape_ops_t;
...
float triangle_area(shape_t *s)         /* 三角形面积,用海伦公式 */
{
...
float triangle_perimeter(shape_t *s)    /* 三角形周长 */
{
    triangle_t *t = (triangle_t *)s;
    float a = t->side_a;
    float b = t->side_b;
...
void triangle_distroy(shape_t *s)
{
    printf("distroy %s\n", s->name);
    triangle_t *t = (triangle_t *)s;
    free(t);
...
float rectangle_area(shape_t *s)        /* 矩形面积 */
{
    rectangle_t *r = (rectangle_t *)s;
    return (r->width * r->height);
}

float rectangle_perimeter(shape_t *s)   /* 矩形周长 */
{
    rectangle_t *r = (rectangle_t *)s;
    return (r->width + r->height)*2;
}

void rectangle_distroy(shape_t *s)
{
    printf("distroy %s\n", s->name);
    rectangle_t *r = (rectangle_t *)s;
    free(r);
...
float circle_area(shape_t *s)           /* 圆面积 */
{
...
float circle_perimeter(shape_t *s)      /* 圆周长 */
{
    circle_t *c = (circle_t *)s;
    return (DEF_PI*c->diameter);
}

void circle_distroy(shape_t *s)
{
    printf("distroy %s\n", s->name);
    circle_t *c = (circle_t *)s;
    free(c);
